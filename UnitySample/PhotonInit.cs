...
public class PhotonInit : MonoBehaviourPunCallbacks
{
    ...
    void MyListRenewal()
    {
        ...
        {
            string str = (multiple + i < myList.Count) ? myList[multiple + i].Name : "";
            string str1 = (multiple + i < myList.Count)
                ? myList[multiple + i].PlayerCount + "/" + myList[multiple + i].MaxPlayers
                : "";   

            string total = string.Concat(str, " ", str1);

            CellBtn[i].interactable = (multiple + i < myList.Count) ? true : false;
            CellBtn[i].transform.GetChild(0).GetComponent<Text>().text = total;
